...
//        self.baseSearchTableView.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;//UIScrollView也适用
//    }else {
...
//    [cancelButton setTitleColor:RGBCOLOR(34, 34, 34) forState:UIControlStateNormal];
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 11.0) { // iOS 11
        _searchBar.py_width = (self.view.py_width - cancelButton.py_width - PYSEARCH_MARGIN * 3 - 8) * 0.93;
        ...
        titleView.py_width = (self.view.py_width - cancelButton.py_width - titleView.py_x * 2 - 3) * 0.93;
...
//    [cancleButton setTitleColor:RGBACOLOR(34,34,34,1) forState:UIControlStateNormal];
    UIBarButtonItem * rightButtonItem = [[UIBarButtonItem alloc] initWithCustomView:cancleButton];
    ...
    cancleButton.py_width += 15;
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 11.0) { // iOS 11
        _searchBar.py_width = (self.view.py_width - cancleButton.py_width - PYSEARCH_MARGIN * 3 - 8) * 0.95;
        _searchBar.py_height = self.view.py_width > self.view.py_height ? 24 : 30;
        _searchTextField.frame = _searchBar.bounds;
    } else {
        UIView *titleView = self.navigationItem.titleView;
        titleView.py_x = PYSEARCH_MARGIN * 1.5;
        titleView.py_y = self.view.py_width > self.view.py_height ? 3 : 7;
        titleView.py_width = (self.view.py_width - cancleButton.py_width - titleView.py_x * 2 - 3) * 0.95;
        titleView.py_height = self.view.py_width > self.view.py_height ? 24 : 30;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.isHome) {
        if (self.searchHistories.count ==0 ){}else{
        [self setEdgesForExtendedLayout:UIRectEdgeNone];
        self.baseSearchTableView.contentInset = UIEdgeInsetsMake(-8, 0, self.view.py_y, 0);
...
+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder isHome:(BOOL)ishome {
    PYSearchViewController *searchVC = [[PYSearchViewController alloc] init];
    searchVC.hotSearches = hotSearches;
    searchVC.isHome = ishome;
...
+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder didSearchBlock:(PYDidSearchBlock)block {
    PYSearchViewController *searchVC = [self searchViewControllerWithHotSearches:hotSearches searchBarPlaceholder:placeholder isHome:true];
...
{
    self.hotSearchTagsContentView.backgroundColor = [UIColor redColor];
...
- (void)keyboardDidShow:(NSNotification *)noti
{
    ...
    self.searchSuggestionVC.tableView.contentInset = UIEdgeInsetsMake(-36, 0, self.keyboardHeight + 30, 0);
}
...
- (void)searchSuggestionViewClose
{
    self.searchBar.text = @"";
    if (PYSearchResultShowModeEmbed == self.searchResultShowMode && self.showSearchResultWhenSearchTextChanged) {
        [self handleSearchResultShow];
        self.searchResultController.view.hidden = 0 == self.searchBar.text.length;
    } else if (self.searchResultController) {
        self.searchResultController.view.hidden = YES;
    }
    self.baseSearchTableView.hidden = self.searchBar.text.length && !self.searchSuggestionHidden && [self.searchSuggestionVC.tableView numberOfRowsInSection:0];
    self.searchSuggestionVC.view.hidden = self.searchSuggestionHidden || !self.searchBar.text.length || ![self.searchSuggestionVC.tableView numberOfRowsInSection:0];
...
//    self.searchSuggestionVC.view.hidden = YES;
    ...
    if ([self.delegate respondsToSelector:@selector(searchViewController:searchTextDidChange:searchText:)]) {
        [self.delegate searchViewController:self searchTextDidChange:self.searchBar searchText:self.searchBar.text];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        cell.textLabel.textColor = mHexColor(0x333333);
...
//{
//    return self.searchHistories.count && self.showSearchHistory && PYSearchHistoryStyleCell == self.searchHistoryStyle ? 25 : 0.1;
//}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (self.keyboardShowing) {
        // Adjust the content inset of suggestion view
        self.searchSuggestionVC.tableView.contentInset = UIEdgeInsetsMake(-36, 0, 30, 0);
        [self.searchBar resignFirstResponder];
    }
}
